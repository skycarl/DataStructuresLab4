...
class Lab4 {
   ...
   public static void main(String[] args) {
      ...
      if (args.length == 2) {
         ...
         summaryOutputFilename = args[1];
              ...
              1) { // Case where we default to default output filenames
         inputFilenames = args[0];
         summaryOutputFilename = "summaryOutput.txt";
      } else if (args.length == 0) {
         System.out.println(
                 "Error: runtime parameters must be specified. At minimum, specify name of file containing input filenames.");
         System.exit(1);
      } else { // Unspecified error with runtime parameters
         System.out.println(
                 "Unspecified error with runtime parameters. Refer to README.md and try again.");
         System.exit(1);
      }
    ...
    */
   private static void outputSummaryData(SortPerformance[] results,
      ...
      try {
         ...
         // Create table header
         headerString =
                 String.format("\n%-10s%21s%21s%21s%21s%21s%21s", "Sort type",
                         "Heap sort (ns)", "Quicksort1 (ns)",
                         "Quicksort2 (ns)", "Quicksort3 (ns)",
                         "Quicksort4 (ns)", "Insertion sort (ns)");
         ...
         for (SortPerformance result : results) {
            ...
            performanceString =
                    String.format("\n%-10s%21s%21s%21s%21s%21s%21s", sortType,
                            result.getHeapSort(),
                            result.getQuickSort1(),
                            result.getQuickSort2(), result.getQuickSort3(),
                            result.getQuickSort4(),
    ...
    */
   private static SortPerformance sortWithAllMethods(int[] unsortedArray,
                                                     String filename) {
      ...
      outputSortedArray(heapSorted, filename, "Heapsorted");
   ...
   private static void outputSortedArray(int[] sorted, String filename,
      ...
      try {
         ...
         FileWriter outWriter = new FileWriter(
                 new File("sortedOutputFiles", outputFilename));
   ...
   private static int[] importDataFile(String filename) {
      ...
      try {
         ...
         Scanner dataFileScanner =
                 new Scanner(new BufferedReader(new FileReader(dataFile)));
   ...
   private static String[] importFileNames(String filename) {
      ...
      try {
         ...
         Scanner fileScanner = new Scanner(
                 new BufferedReader(new FileReader(filenameFile)));
         ...
         // Remove the null entry at the end of the array
         fileNameArray =
                 Arrays.copyOf(fileNameArray, fileNameArray.length - 1);
